...
results_65_plus <- function(results){
  results %<>% 
    ...
    dplyr::select(-one_of(variable_names$prop_dem_Ci_by_age[1:3])) %>%
    ...
    dplyr::select(-one_of(variable_names$prop_dem_Ci_W_by_age[1:3])) %>%
    ...
    dplyr::select(-one_of(variable_names$prop_dem_Ci_M_by_age[1:3])) %>%
